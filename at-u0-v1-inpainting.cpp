...
//  vector<PrimalForm0> m;
//  m.push_back( PrimalForm0( calculus ) );
//  m.push_back( PrimalForm0( calculus ) );
//  m.push_back( PrimalForm0( calculus ) );
//  for ( Index index = 0; index < m[ 0 ].myContainer.rows(); index++)
//  {
//    SCell cell = m[ 0 ].getSCell( index );
//    double  col = ((double) mask( K.sCoords( cell ) )) / 255.0;
//    col > 0.5 ? col = 1.0 : col = 0.0;
//    m[ 0 ].myContainer( index ) = col;
//    m[ 1 ].myContainer( index ) = col;
//    m[ 2 ].myContainer( index ) = col;
...
//  trace.endBlock();
  ...
  savePrimalForms0ToPPMImage( calculus, end_image, diag(calculus,m)*g[ 0 ],
                                                   diag(calculus,m)*g[ 1 ],
                                                   diag(calculus,m)*g[ 2 ], str_image_gm);
  trace.endBlock();
  ...
  // Building alpha_G0_1
  const PrimalIdentity0 alpha_iG0   = a * diag(calculus,m) * Id0;
...
//      savePrimalForm1ToImage( calculus, dbl_image, v, str_image_v );
      ...
      abs_u_minus_g.push_back( PrimalForm0( calculus ) );
      abs_u_minus_g.push_back( PrimalForm0( calculus ) );
      abs_u_minus_g.push_back( PrimalForm0( calculus ) );
      for ( Dimension i = 0; i < 3; ++i )
        {
          abs_u_minus_g[ i ] = u[ i ] - g[ i ];
          for ( Index index = 0; index < abs_u_minus_g[ i ].myContainer.rows(); index++)
            if ( abs_u_minus_g[ i ].myContainer( index ) < 0 )
              abs_u_minus_g[ i ].myContainer( index ) *= -1.0;
        }
      ossUminusG << boost::format("%s-a%.5f-l%.7f-diff.ppm") %f2 %a %l;
      string str_image_u_minus_g = ossUminusG.str();
      savePrimalForms0ToPPMImage( calculus, end_image, abs_u_minus_g[ 0 ],
                                                       abs_u_minus_g[ 1 ],
                                                       abs_u_minus_g[ 2 ], str_image_u_minus_g);
