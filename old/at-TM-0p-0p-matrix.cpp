  ...
  while ( l1 >= l2 )
    {
      ...
      double l = l1;
      trace.info() << "B'B'" << endl;
      const Calculus::PrimalIdentity0 lBB = l * tA_tS_S_A;
      Calculus::PrimalForm0 l_sur_4( calculus );
      ...
      for( int k = 0 ; k < 5 ; ++k )
        {
          if (eps/coef_eps < h*h)
            ...
            {
              eps /= coef_eps;
              Calculus::PrimalIdentity0 BB = eps * lBB + ( l/(4.0*eps) ) * Id0; // tS_S;
              int i = 0;
              for ( ; i < n; ++i )
                {
                  ...
                  Calculus::PrimalIdentity0 diag_v = diag( calculus, v );
                  Calculus::PrimalIdentity0 diag_v2 = square( calculus, diag_v );
                  Calculus::PrimalIdentity1 tS_v2_S = G1;
                  tS_v2_S.myContainer = (tSx * diag_v2.myContainer * Sx
                                         + tSy * diag_v2.myContainer * Sy); 
                  // Calculus::PrimalDerivative0 tS_S_v_A = tS_S * v_A;
                  Calculus::PrimalIdentity0 Av2A = calculus.identity<0, PRIMAL>();
                  Av2A.myContainer = tA * tS_v2_S.myContainer * A
                    + alpha_iG0.myContainer;
                  trace.info() << "Prefactoring matrix Av2A := tA_diag(v^2)_A + alpha_iG0" << endl;
                  trace.info() << "-------------------------------------------------------------------------------" << endl;
                  ...
                  const Calculus::PrimalForm0 former_v = v;
                  trace.beginBlock("Solving for v");
                  trace.info() << "Building matrix BB+Mw2" << endl;
                  const Calculus::PrimalDerivative0 A_u = primal_D0 * diag( calculus, u );
                  Calculus::PrimalIdentity0 tu_tA_tS_S_A_u = calculus.identity<0, PRIMAL>();
                  tu_tA_tS_S_A_u.myContainer = A_u.myContainer.transpose() * tS_S.myContainer * A_u.myContainer; 
                  solver_v.compute( tu_tA_tS_S_A_u + BB );
                  ...
                  trace.info() << "0-form v: min=" << m1 << " avg=" << ( ma/ v.myContainer.rows() )
                               << " max=" << m2 << std::endl;
                  for ( Calculus::Index index = 0; index < v.myContainer.rows(); index++)
      ...
      {
        Image end_v_image = image;
        PrimalForm0ToImage( calculus, v, end_v_image );
        ostringstream ossV;
        //ossV << f2 << "-l" << int_l << "_" << dec_l << "-v.pgm";
        ossV << boost::format("%s-l%.7f-v.pgm") %f2 %l;
        string str_image_v = ossV.str();
        end_v_image >> str_image_v.c_str();
      }
      l1 /= lr;
    }
