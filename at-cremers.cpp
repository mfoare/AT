  ...
  if ( ! parseOK || vm.count("help") || !vm.count("input") )
    {
      cerr << "Usage: " << argv[0] << " -i toto.pgm\n"
                ...
                << " / "
                << endl
                << " | (u-g)^2 + (1/a). v^2 |grad u|^2 + (le/a).|grad v|^2 + (l/4ae).(1-v)^2 "
                << endl
                << " / "
                << endl << endl
                << general_opt << "\n";
...
//      trace.info() << "l_4e" << endl;
				  ...
				  for ( ; i < n; ++i )
				    {
				      ...
				      const Calculus::PrimalIdentity0 Av2A = 	- 1.0 * a * h 	* dual_h2 * dual_D1 * primal_h1 * Mv2 * primal_D0
				        									 										+ 					h*h * calculus.identity<0, PRIMAL>();
				      trace.info() << "Prefactoring matrix Av2A" << endl;
				      solver_u.compute( Av2A );
				      trace.info() << "Solving Av2A u = g" << endl;
				      u = solver_u.solve( h*h * g );
      ...
      for ( Calculus::Index index = 0; index < u.myContainer.rows(); index++)
        UmG2 += (u.myContainer( index ) - g.myContainer( index )) * (u.myContainer( index ) - g.myContainer( index ));
